...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/05/09 12:26:03 by mevelyne          #+#    #+#             */
/*   Updated: 2021/05/09 12:26:08 by mevelyne         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
unsigned int	*take_argument2(unsigned short op_index,
	unsigned int *tmp_pc, t_proc *pr)
{
	unsigned int	*arg;

	arg = malloc(sizeof(unsigned int) * g_tab[op_index].count_arg);
	if (!(arg))
		ft_error(5, NULL);
	if (g_tab[op_index].is_codage)
		*tmp_pc = (pr->pc + 2) % MEM_SIZE;
	else
		*tmp_pc = (pr->pc + 1) % MEM_SIZE;
	return (arg);
}
								...
								t_proc *pr, unsigned short op_index)
{
	...
	arg = take_argument2(op_index, &tmp_pc, pr);
	j = 8;
	i = -1;
